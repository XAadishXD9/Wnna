...
async def sendvps(
    ...
    embed.set_footer(text="🔐 Safe your details | Powered by EagleNode")
...
@bot.tree.command(name="addadmin", description="👑 Add a new admin user ID (Admin only)")
@app_commands.describe(user_id="The Discord user ID to add as admin")
async def add_admin(interaction: discord.Interaction, user_id: int):
    ...
    if user_id in ADMIN_IDS:
        await interaction.response.send_message(
            f"⚠️ <@{user_id}> is already an admin!", ephemeral=True
        )
        return

    ADMIN_IDS.append(user_id)

    # Optionally persist ADMIN_IDS to a file named admins.txt
    try:
        with open("admins.txt", "w") as f:
            f.write(",".join(str(x) for x in ADMIN_IDS))
    except Exception:
        pass

    await interaction.response.send_message(
        f"✅ Successfully added <@{user_id}> as a new admin!", ephemeral=False
    )


@bot.tree.command(name="removeadmin", description="🗑️ Remove an existing admin user ID (Admin only)")
@app_commands.describe(user_id="The Discord user ID to remove from admin list")
async def remove_admin(interaction: discord.Interaction, user_id: int):
    ...
    if user_id == interaction.user.id:
        await interaction.response.send_message(
            "⚠️ You cannot remove yourself from the admin list!", ephemeral=True
        )
        return
    ...
    if user_id in ADMIN_IDS:
        ...
        await interaction.response.send_message(
            f"✅ Successfully removed <@{user_id}> from the admin list.", ephemeral=False
        )
    else:
        await interaction.response.send_message(
            f"❌ <@{user_id}> is not an admin.", ephemeral=True
        )


# -------------------- Manage VPS command (admin) --------------------
class ManageVPSView(View):
    ...
    async def start_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await start_server(interaction, self.container_name)

    @discord.ui.button(label="⏹️ Stop", style=discord.ButtonStyle.secondary)
    async def stop_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await stop_server(interaction, self.container_name)

    @discord.ui.button(label="🔄 Restart", style=discord.ButtonStyle.primary)
    async def restart_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await restart_server(interaction, self.container_name)

    @discord.ui.button(label="🗑️ Delete", style=discord.ButtonStyle.danger)
    async def delete_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        confirm_embed = discord.Embed(
            title="⚠️ Confirm Deletion",
            description=f"Are you sure you want to delete `{self.container_name}`?",
            color=0x2400ff
        )
        view = ConfirmView(self.container_name, self.container_name)
        await interaction.response.send_message(embed=confirm_embed, view=view)


@bot.tree.command(name="manage_vps", description="⚙️ Admin: Manage a specific VPS by container name or ID")
@app_commands.describe(vps_id="The container name or ID of the VPS to manage")
async def manage_vps(interaction: discord.Interaction, vps_id: str):
    ...
    # Get stats and ssh command
    stats = get_container_stats(vps_id)
    ssh_cmd = get_ssh_command_from_database(vps_id)

    # If container not found in database and docker inspect fails, still show stats if available
    if not ssh_cmd and stats.get("status") == "🔴 Stopped":
        # Allow admins to manage containers that exist but have no DB entry
        try:
            # Try to get container status via docker inspect
            subprocess.check_output(["docker", "inspect", "--format", "{{.State.Status}}", vps_id])
        except Exception:
            embed = discord.Embed(
                title="❌ Not Found",
                description=f"No VPS found with ID `{vps_id}`.",
                color=0x2400ff
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
    ...
    embed = discord.Embed(
        title=f"🖥️ Manage VPS: {vps_id}",
        description="Select an action below to manage this VPS.",
        color=0x2400ff
    )
    embed.add_field(name="💾 Memory", value=stats.get("memory", "N/A"), inline=True)
    embed.add_field(name="🔥 CPU", value=stats.get("cpu", "N/A"), inline=True)
    embed.add_field(name="🟢 Status", value=stats.get("status", "N/A"), inline=True)
    if ssh_cmd:
        embed.add_field(name="🔑 SSH Command", value=f"```{ssh_cmd}```", inline=False)
    view = ManageVPSView(vps_id)
    await interaction.response.send_message(embed=embed, view=view)
