...
def get_container_stats(container_id):
    try:
        ...
        return {
            "memory": mem_stats,
            "cpu": cpu_stats,
            "status": "ðŸŸ¢ Running" if status == "running" else "ðŸ”´ Stopped"
        }
    except Exception:
        return {"memory": "N/A", "cpu": "N/A", "status": "ðŸ”´ Stopped"}
...
class OSSelectView(View):
    def __init__(self, callback):
        ...
        select = Select(
            ...
            options=[
                discord.SelectOption(label="Ubuntu 22.04", description="Latest LTS Ubuntu release", emoji="ðŸ§", value="ubuntu"),
                discord.SelectOption(label="Debian 12", description="Stable Debian release", emoji="ðŸ§", value="debian")
            ]
        )
...
async def on_ready():
    change_status.start()
    print(f'ðŸš€ Bot is ready. Logged in as {bot.user}')
    await bot.tree.sync()
...
@bot.tree.command(name="nodedmin", description="ðŸ“Š Admin: Lists all VPSs, their details, and SSH commands")
    ...
    for line in lines:
        ...
        if field_count >= 25:
            ...
            current_embed = discord.Embed(
                title="ðŸ“Š All VPS Instances (Continued)",
                description="Detailed information about all VPS instances",
                color=0x2400ff
            )
            field_count = 0
        ...
        if len(parts) >= 8:
            ...
            current_embed.add_field(
                name=f"ðŸ–¥ï¸ {container_name} ({stats['status']})",
                value=f"ðŸª© **User:** {user}\n"
                      f"ðŸ’¾ **RAM:** {ram}GB\n"
                      f"ðŸ”¥ **CPU:** {cpu} cores\n"
                      f"ðŸŒ **OS:** {os_type}\n"
                      f"ðŸ‘‘ **Creator:** {creator}\n"
                      f"ðŸ”‘ **SSH:** `{ssh_command}`",
                inline=False
            )
            field_count += 1
        elif len(parts) >= 3:
...
async def node_stats(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        title="ðŸ“Š Panel Node Dashboard",
        description="ðŸ“¡ EagleNode",
        color=0x2400ff
    )
...
async def regen_ssh_command(interaction: discord.Interaction, container_name: str):
    ...
    if ssh_session_line:
        ...
        dm_embed.add_field(
            name="ðŸ”‘ SSH Connection Command",
...
async def port_add(interaction: discord.Interaction, container_name: str, container_port: int):
    embed = discord.Embed(
        title="ðŸ”„ Setting Up IPV4 Forwarding",
...
async def port_forward_website(interaction: discord.Interaction, container_name: str, container_port: int):
    embed = discord.Embed(
        title="ðŸ”„ Setting Up HTTP Forwarding",
...
async def deploy(
    ...
    # Validate parameters
    if ram > 200:
        ram = 200  # Cap at 100GB
    if cpu > 100:
        cpu = 100  # Cap at 24 cores
    ...
    embed = discord.Embed(
        title="**ðŸ–¥ï¸ Select Operating System**",
        description="** ðŸ” Please select the operating system for your VPS instance **",
        color=0x2400ff
    )
...
async def deploy_with_os(interaction, os_type, ram, cpu, user_id, user, container_name, expiry_date):
    ...
    embed = discord.Embed(
        ...
        description=f"**ðŸ’¾ RAM: {ram}GB\n**"
                    f"**ðŸ”¥ CPU: {cpu} cores\n**"
                    f" ðŸ§Š**OS:** {os_type}\n"
                    f"**ðŸ§Š conatiner name: {user}\n**"
    ...
    if ssh_session_line:
        ...
        dm_embed.add_field(name="ðŸ”‘ SSH Connection Command", value=f"```{ssh_session_line}```", inline=False)
        dm_embed.add_field(name="ðŸ’¾ RAM Allocation", value=f"{ram}GB", inline=True)
        dm_embed.add_field(name="ðŸ”¥ CPU Cores", value=f"{cpu} cores", inline=True)
        dm_embed.add_field(name="ðŸ§Š Container Name", value=container_name, inline=False)
        dm_embed.add_field(name="ðŸ’¾ Storage", value=f"1000 GB (Shared storage)", inline=True)
        dm_embed.add_field(name="ðŸ”’ Password", value="eagle@123", inline=False)
        ...
        try:
            ...
            success_embed = discord.Embed(
                ...
                description=f"** ðŸŽ‰ VPS instance has been created for <@{user_id}>. They should check their DMs for connection details.**",
                color=0x2400ff
            )
            await interaction.followup.send(embed=success_embed)
            ...
            warning_embed.add_field(name="ðŸ”‘ SSH Connection Command", value=f"```{ssh_session_line}```", inline=False)
            await interaction.followup.send(embed=warning_embed)
    else:
...
class TipsView(View):
    def __init__(self):
        ...
        self.tips = [
            ...
            {
                "title": "ðŸ“Š Performance Optimization",
...
@bot.tree.command(name="tips", description="ðŸ’¡ Shows useful tips for managing your VPS")
...
async def list_servers(interaction: discord.Interaction):
    ...
    for server in servers:
        ...
        try:
            ...
            status = "ðŸŸ¢ Running" if container_info == "running" else "ðŸ”´ Stopped"
        except:
            status = "ðŸ”´ Stopped"
        ...
        if len(parts) >= 8:
            ...
            embed.add_field(
                ...
                value=f"ðŸ’¾ **RAM:** 16GB\n"
                      f"ðŸ”¥ **CPU:** 40 core\n"
                      f"ðŸ’¾ **Storage:** 10000 GB (Shared)\n"
                      f"ðŸ§Š **OS:** Ubuntu 22.04",
                inline=False
            )

    await interaction.followup.send(embed=embed)

@bot.tree.command(name="sendvps", description="ðŸ‘‘ Admin: Send VPS details to a user via DM")
@app_commands.describe(
    ram="RAM in GB",
    cpu="CPU cores",
    ip="IP address",
...
async def sendvps(
    ...
    embed.add_field(name="ðŸŒ IP", value=ip, inline=True)
    embed.add_field(name="ðŸ”Œ Port", value=port, inline=True)
    embed.add_field(name="ðŸ”’ Password", value=password, inline=True)
    embed.add_field(name="ðŸ§¬ Full Combo", value=f"```{fullcombo}```", inline=False)
    embed.add_field(name="ðŸ’¾ RAM", value=f"{ram} GB", inline=True)
    embed.add_field(name="ðŸ”¥ CPU", value=f"{cpu} cores", inline=True)
    embed.set_footer(text="ðŸ” Safe your details | Powered by EagleNode")
    ...
    def __init__(self, user: discord.Member):
        ...
        self.add_item(Select(
            ...
            options=[
                ...
                discord.SelectOption(label="Boost Reward", value="boost", emoji="ðŸŽ")
            ]
        ))
    ...
    async def select_callback(self, interaction: discord.Interaction, select: Select):
        ...
        if choice == "invite":
            ...
            if reward:
                ...
                await interaction.response.send_message(f"âŒ You have only **{user_invites} invites**. You need at least **8** to claim.", ephemeral=True)
            ...
            if reward:
                ...
                await interaction.response.send_message(f"âŒ You need at least **2 boosts** to claim. Current: {boost_count}", ephemeral=True)
@bot.tree.command(name="create", description="ðŸŽ Request a VPS via Invite or Boost rewards")
async def create(interaction: discord.Interaction):
    if not interaction.guild:
        await interaction.response.send_message("âŒ You must use this in a server.", ephemeral=True)
        return

    view = RewardSelectView(interaction.user)
    embed = discord.Embed(
        title="ðŸŽ‰ VPS Reward Selection",
...
async def send_vps_request(interaction, user, method, reward, count):
    ...
    if not channel:
        await interaction.response.send_message("âŒ VPS channel not found.", ephemeral=True)
        return

    embed = discord.Embed(
        title="ðŸš€ VPS Request Submitted",
        description=f"User: {user.mention}\nMethod: {method} Reward",
        color=0x2400ff
    )
    embed.add_field(name="ðŸ“Š RAM", value=f"{reward['ram']} GB", inline=True)
    embed.add_field(name="ðŸ”¥ CPU", value=f"{reward.get('cpu', 2)} cores", inline=True)
