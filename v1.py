...
def get_system_stats():
    try:
        total_mem_out = subprocess.check_output(["free", "-m"]).decode().strip()
        mem_lines = total_mem_out.split('\n')
        total_mem, used_mem = "N/A", "N/A"
        if len(mem_lines) >= 2:
            mem_values = mem_lines[1].split()
            total_mem, used_mem = mem_values[1], mem_values[2]
        disk_usage = subprocess.check_output(["df", "-h", "/"]).decode().strip()
        disk_lines = disk_usage.split('\n')
        total_disk, used_disk = "N/A", "N/A"
        if len(disk_lines) >= 2:
            disk_values = disk_lines[1].split()
            total_disk, used_disk = disk_values[1], disk_values[2]
        return {"total_memory": f"{total_mem}GB", "used_memory": f"{used_mem}GB", "total_disk": total_disk, "used_disk": used_disk}
    except Exception as e:
        return {"total_memory":"N/A","used_memory":"N/A","total_disk":"N/A","used_disk":"N/A","error":str(e)}

async def capture_ssh_session_line(process):
    """
    Improved tmate SSH session capture — waits up to 30s and detects multiple formats.
    Returns the ssh command string (e.g. 'ssh user@...') or None on timeout.
    """
    ssh_line = None
    start_time = time.time()
    # read lines until SSH line detected or timeout
    while True:
        ...
        try:
            ...
            # allow loop to continue waiting
            continue
        if not output:
            await asyncio.sleep(0.5)
            continue
        line = output.decode('utf-8', errors='ignore').strip()
        # debug print (helpful when running bot locally; remove or comment if noisy)
        print(f"[tmate] {line}")
        # search for common patterns
        # pattern: "ssh session: <ssh command>"
        if "ssh session:" in line.lower():
            # split on the lowercase keyword to be safe
            parts = re.split("ssh session:", line, flags=re.IGNORECASE)
            if len(parts) >= 2:
                ssh_line = parts[-1].strip()
                break
        # pattern: line containing "ssh " and an host
        if re.search(r"\bssh\s+[^\s]+", line):
            # return from first 'ssh ...' occurrence
            idx = line.lower().find("ssh ")
            ssh_line = line[idx:].strip()
            break
        # sometimes tmate prints URLs (web) first — keep waiting
        # continue loop
    return ssh_line
...
async def nodedmin(interaction: discord.Interaction):
    if not is_admin(interaction.user.id):
        await interaction.response.send_message(embed=discord.Embed(title="❌ Access Denied", description="You don't have permission to use this command.", color=0x2400ff), ephemeral=True)
        return
    await interaction.response.defer()
    if not os.path.exists(database_file):
        await interaction.followup.send(embed=discord.Embed(title="VPS Instances", description="No VPS data available.", color=0x2400ff))
        return
    with open(database_file, 'r') as f:
        lines = f.readlines()
    if not lines:
        ...
        return
    embeds = []
    current_embed = discord.Embed(title="All VPS Instances", description="Detailed information about all VPS instances", color=0x2400ff)
    field_count = 0
    for line in lines:
        parts = line.strip().split('|')
        if field_count >= 24:
            embeds.append(current_embed)
            current_embed = discord.Embed(title="📊 All VPS Instances (Continued)", description="Detailed information about all VPS instances", color=0x2400ff)
            field_count = 0
        if len(parts) >= 8:
            user, container_name, ssh_command, ram, cpu, creator, os_type, expiry = parts
            stats = get_container_stats(container_name)
            current_embed.add_field(name=f"🖥️ {container_name} ({stats['status']})", value=f"🪩 **User:** {user}\n💾 **RAM:** {ram}GB\n🔥 **CPU:** {cpu} cores\n🌐 **OS:** {os_type}\n👑 **Creator:** {creator}\n🔑 **SSH:** `{ssh_command}`", inline=False)
            field_count += 1
        elif len(parts) >= 3:
...
async def node_stats(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="📊 Panel Node Dashboard", description="📡 EAGLENODE", color=0x2400ff)
    embed.add_field(name="🔥 Memory Usage", value=f"Used: {system_stats['used_memory']} / Total: {system_stats['total_memory']}", inline=False)
    embed.add_field(name="💾 Storage Usage", value=f"Used: {system_stats['used_disk']} / Total: {system_stats['total_disk']}", inline=False)
    embed.add_field(name=f"💙 VPS ({len(containers)})", value="List of all VPS instances and their status:", inline=False)
    for container_info in containers:
        parts = container_info.split('|')
        if len(parts) >= 2:
            container_id = parts[1]
            stats = get_container_stats(container_id)
            embed.add_field(name=f"{container_id}", value=f"Status: {stats['status']}\nMemory: {stats['memory']}\nCPU: {stats['cpu']}", inline=True)
    await interaction.followup.send(embed=embed)

# -----------------------
# Deploy (instant) with waiting embed for SSH
# -----------------------
@bot.tree.command(name="deploy", description="🚀 Admin: Instantly deploy a VPS")
@app_commands.describe(user="The Discord user to assign the VPS to", os="Operating system (ubuntu or debian)", ram="RAM in GB (e.g. 4)", cpu="CPU cores (e.g. 2)")
async def deploy(interaction: discord.Interaction, user: discord.User, os: str, ram: int, cpu: int):
    ...
    # initial creation message
    creating_embed = discord.Embed(title="⚙️ Creating VPS Instance", description=f"👤 **User:** {user.mention}\n🐧 **OS:** {os}\n💾 **RAM:** {ram}GB\n🔥 **CPU:** {cpu} cores", color=0x2400ff)
    await interaction.response.send_message(embed=creating_embed)
    ...
    try:
        ...
        await interaction.followup.send(embed=discord.Embed(title="❌ Error", description=f"Error creating Docker container: {e}", color=0xff0000))
        return

    # show waiting embed while capturing ssh
    waiting_embed = discord.Embed(title="⏳ Waiting for SSH session...", description="tmate is starting inside the container. This can take a few seconds — waiting for the SSH session line.", color=0x00aaff)
    waiting_message = await interaction.followup.send(embed=waiting_embed)

    try:
        # exec tmate
        exec_proc = await asyncio.create_subprocess_exec("docker", "exec", container_name, "tmate", "-F", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT)
        ssh_session_line = await capture_ssh_session_line(exec_proc)
    except Exception as e:
        ssh_session_line = None
        print("tmate exec error:", e)

    if not ssh_session_line:
        # cleanup - remove container to avoid orphaned containers if desired
        try:
            subprocess.run(["docker", "stop", container_name], check=False)
            subprocess.run(["docker", "rm", container_name], check=False)
        except Exception:
            pass
        await waiting_message.edit(embed=discord.Embed(title="❌ Deployment Failed", description="Failed to get SSH session from tmate. Container removed.", color=0xff0000))
        return

    # store data
    add_to_database(str(user), container_name, ssh_session_line, ram_limit=ram, cpu_limit=cpu, creator=str(interaction.user), expiry=expiry_date, os_type=os_type_to_display_name(os))

    # final DM to user
    dm_embed = discord.Embed(title="✅ VPS Created Successfully!", description="Here are your VPS details:", color=0x2400ff)
    ...
    dm_embed.set_footer(text="🔐 Powered by EAGLE NODE")

    try:
        await user.send(embed=dm_embed)
        await waiting_message.edit(embed=discord.Embed(title="✅ VPS Created", description=f"Successfully created VPS for {user.mention}. Check your DMs for SSH details.", color=0x00ff00))
    except discord.Forbidden:
        # Can't DM user
        await waiting_message.edit(embed=discord.Embed(title="⚠️ VPS Created (DM failed)", description=f"VPS created for {user.mention}, but I couldn't DM them. SSH command:\n```{ssh_session_line}```", color=0xffa500))
...
async def port_add(interaction: discord.Interaction, container_name: str, container_port: int):
    await interaction.response.send_message(embed=discord.Embed(title="🔄 Setting Up IPV4 Forwarding", description="Setting up port forwarding. This might take a moment...", color=0x2400ff))
    ...
    try:
        ...
        success_embed = discord.Embed(title="✅ Get IPV4 Successful", description=f"Your service is now accessible from the internet.\n**Host:** {PUBLIC_IP}\n**Port:** {public_port}", color=0x2400ff)
        await interaction.followup.send(embed=success_embed)
    except Exception as e:
        await interaction.followup.send(embed=discord.Embed(title="❌ Error", description=f"An unexpected error occurred: {e}", color=0xff0000))
...
async def port_forward_website(interaction: discord.Interaction, container_name: str, container_port: int):
    await interaction.response.send_message(embed=discord.Embed(title="🔄 Setting Up HTTP Forwarding", description="Setting up HTTP forwarding. This might take a moment...", color=0x2400ff))
    try:
        ...
        if url_line:
            ...
            success_embed = discord.Embed(title="✅ HTTP Forwarding Successful", description=f"Your web service is now accessible from the internet.", color=0x2400ff)
            success_embed.add_field(name="🌐 Website URL", value=f"[{url}](https://{url})", inline=False)
            await interaction.followup.send(embed=success_embed)
        else:
            await interaction.followup.send(embed=discord.Embed(title="❌ Error", description="Failed to set up HTTP forwarding. Please try again later.", color=0xff0000))
    except Exception as e:
        await interaction.followup.send(embed=discord.Embed(title="❌ Error", description=f"An unexpected error occurred: {e}", color=0xff0000))
...
async def manage(interaction: discord.Interaction, container_id: str):
    ...
    for vps in vps_list:
        if container_id in vps:
            ...
            break
    if not vps_data:
        await interaction.followup.send(embed=discord.Embed(title="❌ Not Found", description=f"No VPS found with ID or name `{container_id}`.", color=0xff0000), ephemeral=True)
        return
    user, container_name, ssh_command, ram, cpu, creator, os_type, expiry = (vps_data + ["Unknown"] * (8 - len(vps_data)))
    stats = get_container_stats(container_name)
    embed = discord.Embed(title=f"🖥️ VPS Management - {container_name}", description=f"Managing container for **{user}**", color=0x2400ff)
    embed.add_field(name="📊 Resources", value=f"**Plan:** Custom\n**Status:** {stats['status']}\n**RAM:** {ram}GB\n**CPU:** {cpu} cores\n**Storage:** ∞\n**OS:** {os_type}", inline=False)
...
async def delete_server(interaction: discord.Interaction, container_name: str):
    ...
    if not container_id:
        await interaction.response.send_message(embed=discord.Embed(title="❌ Not Found", description="No instance found with that name for your user.", color=0x2400ff))
        return
    try:
        subprocess.run(["docker", "stop", container_id], check=False)
        subprocess.run(["docker", "rm", container_id], check=False)
        remove_from_database(container_id)
        await interaction.response.send_message(embed=discord.Embed(title=" VPS Deleted", description=f"Successfully deleted VPS instance `{container_name}`.", color=0x2400ff))
    except Exception as e:
        await interaction.response.send_message(embed=discord.Embed(title="❌ Error", description=f"Failed to delete VPS instance: {e}", color=0x2400ff))
...
async def delete_all_servers(interaction: discord.Interaction):
    if interaction.user.id not in ADMIN_IDS:
        ...
        return
    containers = get_all_containers()
    confirm_embed = discord.Embed(title="**⚠️ Confirm Mass Deletion**", description=f"**Are you sure you want to delete ALL {len(containers)} VPS instances? This action cannot be undone.**", color=0x2400ff)
...
async def list_servers(interaction: discord.Interaction):
    ...
    for server in servers:
        ...
        if len(parts) >= 8:
            ...
            embed.add_field(name=f"🖥️ {container_id} ({status})", value=f"💾 **RAM:** {ram_limit}GB\n🔥 **CPU:** {cpu_limit} cores\n💾 **Storage:** 10000 GB (Shared)\n 🧊**OS:** {os_type}\n👑 **Created by:** {creator}\n⏱️ **Expires:** {expiry}", inline=False)
        else:
            embed.add_field(name=f"🖥️ {container_id} ({status})", value=f"💾 **RAM:** 16GB\n🔥 **CPU:** 40 core\n💾 **Storage:** 10000 GB (Shared)\n🧊 **OS:** Ubuntu 22.04", inline=False)
    await interaction.followup.send(embed=embed)
...
async def ping(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=discord.Embed(title="🏓 Pong!", description=f"Latency: {latency}ms", color=0x00ff00))
...
class RewardSelectView(View):
    def __init__(self, user: discord.Member):
        super().__init__(timeout=60)
        self.user = user
        self.add_item(Select(placeholder="Select your reward method", options=[discord.SelectOption(label="Invite Reward", value="invite", emoji="✉️"), discord.SelectOption(label="Boost Reward", value="boost", emoji="🎁")]))
...
async def create(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="🎉 VPS Reward Selection", description="Please select your reward method below.", color=0x2400ff)
...
async def send_vps_request(interaction, user, method, reward, count):
    ...
    if not channel:
        ...
        return
    embed = discord.Embed(title="🚀 VPS Request Submitted", description=f"User: {user.mention}\nMethod: {method} Reward", color=0x2400ff)
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(title="**🌟 VPS Bot Help**", description="** Here are all the available commands:**", color=0x00aaff)
    ...
    if interaction.user.id in ADMIN_IDS:
        embed.add_field(name="👑 Admin Commands", value="Commands available only to admins:", inline=False)
