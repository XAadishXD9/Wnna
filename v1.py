...
def get_container_status(container_id):
    try:
        ...
        return "ðŸŸ¢ Running" if out == "running" else "ðŸ”´ Stopped"
    except Exception:
        return "ðŸ”´ Stopped"
...
# Commands
# -------------------------
@bot.tree.command(name="ping", description="ðŸ“ Check the bot's latency")
...
async def node_stats(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="ðŸ“Š Node Dashboard", color=0x2400ff)
...
async def list_servers(interaction: discord.Interaction):
    ...
    await interaction.response.defer()
    if not servers:
        await interaction.followup.send(embed=discord.Embed(title="ðŸ“‹ Your VPS", description="You don't have any VPS instances. Use `/deploy` to create one.", color=0x2400ff))
        return

    embed = discord.Embed(title="ðŸ“‹ Your VPS", description=f"You have {len(servers)} VPS instance(s):", color=0x2400ff)
    for s in servers:
        parts = s.split('|')
        if len(parts) >= 8:
            _, cid, ssh, ram, cpu, creator, os_type, expiry = parts
            status = get_container_status(cid)
            embed.add_field(name=f"{cid} ({status})", value=f"RAM: {ram}GB â€¢ CPU: {cpu} cores\nOS: {os_type}\nExpires: {expiry}", inline=False)
        else:
            _, cid, ssh = parts[:3]
            status = get_container_status(cid)
...
@bot.tree.command(name="vps_list", description="ðŸ”¹ Compact: show your VPS instances")
async def vps_list(interaction: discord.Interaction):
    ...
    for s in servers:
        ...
        if len(parts) >= 8:
            _, cid, ssh, ram, cpu, creator, os_type, expiry = parts
            status = get_container_status(cid)
            lines.append(f"`{cid}` â€” {status} â€” {ram}GB / {cpu} core(s) â€” {os_type}")
        else:
            _, cid, ssh = parts[:3]
            status = get_container_status(cid)
            lines.append(f"`{cid}` â€” {status}")
...
class ManageView(View):
    ...
    @discord.ui.button(label="ðŸŸ¢ Start", style=discord.ButtonStyle.success)
    async def start_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not (is_admin(interaction.user.id) or str(interaction.user) == self.owner_user_str):
            await interaction.response.send_message("âŒ You don't have permission to manage this VPS.", ephemeral=True)
            return
        await interaction.response.defer()
        try:
            subprocess.run(["docker", "start", self.container_id], check=True, stderr=subprocess.DEVNULL)
            await interaction.followup.send(f"âœ… Started `{self.container_id}`")
        except Exception as e:
            await interaction.followup.send(f"âŒ Failed to start: {e}")

    @discord.ui.button(label="ðŸ”´ Stop", style=discord.ButtonStyle.danger)
    async def stop_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not (is_admin(interaction.user.id) or str(interaction.user) == self.owner_user_str):
            await interaction.response.send_message("âŒ You don't have permission to manage this VPS.", ephemeral=True); return
        await interaction.response.defer()
        try:
            subprocess.run(["docker", "stop", self.container_id], check=True, stderr=subprocess.DEVNULL)
            await interaction.followup.send(f"â¹ï¸ Stopped `{self.container_id}`")
        except Exception as e:
            await interaction.followup.send(f"âŒ Failed to stop: {e}")

    @discord.ui.button(label="ðŸ” Restart", style=discord.ButtonStyle.primary)
    async def restart_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.followup.send(f"ðŸ” Restarted `{self.container_id}`")
        except Exception as e:
            await interaction.followup.send(f"âŒ Failed to restart: {e}")

    @discord.ui.button(label="ðŸ§± Reinstall OS", style=discord.ButtonStyle.secondary)
...
class ReinstallSelectView(View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if not rec:
            await interaction.followup.send("âŒ Container record not found in database.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.followup.send(f"âŒ Failed to create container: {e}", ephemeral=True)
            return
        ...
        if ssh_line:
            ...
            if discord_user:
                try:
                    dm = discord.Embed(title="ðŸ” Reinstall Complete", description=f"Your VPS `{self.container_id}` has been reinstalled with `{selected}`.", color=0x2400ff)
                    dm.add_field(name="ðŸ”‘ SSH", value=f"```{ssh_line}```", inline=False)
                    await discord_user.send(embed=dm)
                except discord.Forbidden:
                    pass
            await interaction.followup.send(f"âœ… Reinstalled `{self.container_id}` with `{selected}`. New SSH sent via DM if possible.", ephemeral=True)
        else:
            await interaction.followup.send("âŒ Reinstall succeeded but failed to get SSH session. Check container logs.", ephemeral=True)

@bot.tree.command(name="manage", description="ðŸ§© Open management panel for a VPS (use container id/name)")
...
async def manage(interaction: discord.Interaction, container_id: str):
    ...
    if not rec:
        await interaction.response.send_message("âŒ Container not found in database.", ephemeral=True)
        return
    ...
    status = get_container_status(cid)
    embed = discord.Embed(title=f"VPS Management â€” {cid}", color=0x00aaff)
...
@bot.tree.command(name="regen-ssh", description="ðŸ”„ Regenerate SSH session for a container (by container_id)")
...
async def regen_ssh_cmd(interaction: discord.Interaction, container_id: str):
    ...
    owner_user = rec[0]
    if not (is_admin(interaction.user.id) or str(interaction.user) == owner_user or interaction.user.id == int(owner_user) if owner_user.isdigit() else False):
        await interaction.response.send_message("âŒ You don't have permission to regenerate SSH for this VPS.", ephemeral=True)
        return
    ...
    if ssh_line:
        ...
        try:
            ...
            dm = discord.Embed(title="ðŸ”„ New SSH Session Generated", description="A new SSH session (tmate) has been created for your VPS.", color=0x2400ff)
            dm.add_field(name="ðŸ”‘ SSH", value=f"```{ssh_line}```", inline=False)
            await target.send(embed=dm)
        except discord.Forbidden:
            pass
        await interaction.followup.send("âœ… New SSH session generated and sent via DM (if possible).", ephemeral=True)
    else:
        await interaction.followup.send("âŒ Failed to generate new SSH session.", ephemeral=True)

@bot.tree.command(name="deploy", description="ðŸš€ Admin: Deploy a new VPS instance")
...
async def deploy(interaction: discord.Interaction, ram: int = 2, cpu: int = 1, target_user: str = None, container_name: str = None, expiry: str = None):
    ...
    await interaction.followup.send("Select OS for new VPS:", view=view, ephemeral=True)
...
async def deploy_with_os(interaction, os_type, ram, cpu, user_id, user_str, container_name, expiry_date):
    ...
    try:
        ...
        await interaction.followup.send(f"âŒ Error creating container: {e}", ephemeral=True)
        return
    ...
    if ssh_line:
        ...
        if target_user_obj:
            dm = discord.Embed(title="âœ… VPS Created", description=f"Your VPS `{container_name}` is ready.", color=0x2400ff)
            dm.add_field(name="ðŸ”‘ SSH", value=f"```{ssh_line}```", inline=False)
            ...
            await interaction.followup.send(f"âœ… VPS created for <@{user_id}>. DM sent.", ephemeral=True)
        else:
            await interaction.followup.send(f"âœ… VPS created as `{container_name}`, but couldn't DM the user.", ephemeral=True)
    else:
        subprocess.run(["docker", "stop", container_name], check=False)
        subprocess.run(["docker", "rm", container_name], check=False)
        await interaction.followup.send("âŒ Failed to obtain SSH session from container. Cleanup done.", ephemeral=True)
...
async def delete_server(interaction: discord.Interaction, container_name: str):
    ...
    if not rec:
        await interaction.response.send_message("âŒ Not found.", ephemeral=True); return
    owner = rec[0]
    if not (is_admin(interaction.user.id) or owner == user or (owner.isdigit() and int(owner) == interaction.user.id)):
        await interaction.response.send_message("âŒ You don't have permission to delete this VPS.", ephemeral=True); return
    ...
    await interaction.followup.send(f"âœ… `{container_name}` deleted.", ephemeral=True)

@bot.tree.command(name="delete-all", description="ðŸ—‘ï¸ Admin: Delete all VPS instances")
async def delete_all_servers(interaction: discord.Interaction):
    if interaction.user.id not in ADMIN_IDS:
        await interaction.response.send_message("âŒ Access Denied", ephemeral=True); return
    ...
    await interaction.followup.send(f"âœ… Deleted {deleted} containers.", ephemeral=True)
...
@bot.tree.command(name="port-add", description="ðŸ”Œ Adds a port forwarding rule")
...
@bot.tree.command(name="port-http", description="ðŸŒ Forward HTTP traffic to your container")
